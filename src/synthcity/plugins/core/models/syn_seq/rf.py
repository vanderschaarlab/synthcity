...
def syn_rf(
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        X: 2D array-like of shape (n, p). Covariates for the training data.
    ...
    # Heuristic: if we had categories or dtype is object => classification
    if y_categories is not None or pd.api.types.is_object_dtype(y_dtype):
        ...
        for t_idx in range(n_estimators):
            ...
            for i, leaf_id in enumerate(train_leaves[:, t_idx]):
                leaf_dict.setdefault(leaf_id, []).append(i)
        ...
        for row_idx in range(len(Xp)):
            ...
            for t_idx in range(n_estimators):
                ...
                donors_union.extend(leaf_map[t_idx].get(leaf_id, []))
