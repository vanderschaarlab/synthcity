...
class SurvivalBayesianNetworkPlugin(Plugin):
    """Survival BayesianNetwork plugin.

    Example:
        >>> from synthcity.plugins import Plugins
        >>> X = load_rossi()
        >>> plugin = Plugins().get("survival_bayesian_network", target_column = "arrest", time_to_event_column="week")
        >>> plugin.fit(X)
        >>> plugin.generate()
    """
    ...
    @staticmethod
    def name() -> str:
        return "survival_bayesian_network"
    ...
    def hyperparameter_space(**kwargs: Any) -> List[Distribution]:
        return plugins.Plugins().get_type("bayesian_network").hyperparameter_space()

    def _fit(
        self, X: pd.DataFrame, *args: Any, **kwargs: Any
    ) -> "SurvivalBayesianNetworkPlugin":
...
plugin = SurvivalBayesianNetworkPlugin
