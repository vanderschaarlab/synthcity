...
def test_syn_seq_encoder_prepare() -> None:
    """
    Check .prepare() => sets up syn_order, method, variable_selection
    """
    df = pd.DataFrame(
        {"col1": [10, 20, 30], "col2": [100, 200, 300], "col3": ["A", "B", "C"]}
    )
    enc = Syn_SeqEncoder(
...
def test_syn_seq_encoder_methods_on_small_df() -> None:
    """
    Minimal check that .prepare() doesn't crash on small DF
    """
    df = pd.DataFrame({"X": [1, 2, 3]})
    enc = Syn_SeqEncoder(syn_order=["X"], method={"X": "norm"}, default_method="cart")
    enc.prepare(df)
    info = enc.get_info()
